...
var APIKey = "-H \"APIKey: yQLhgMDaVKrdr7YpTsyUbreJcxgux5zwEPlMhxjraw7gp2DmzTKsyrrRzcgBVIPxkgdokxwuff0bisYkUV0wxf3I2nqM964yjdzzJarbelLHhrkJJYTgKywFnabnwChr\"";
...
var endpoint = "--url \"http://10.4.110.61:8080/CiiMS/web/OAFRestService\"";
...
if (Event.SourceType == "MACRO" && Event.Action == "RUN" && Event.SourceId == "2200") {
    ...
    // DebugLogString("No Computer!");

    var event_uuid = Event.GetParam("source_guid");

    //if (!empty(slave_id) && !empty(event_uuid)) {
    if (!empty(event_uuid)) {

        var transaction_id = Event.GetParam("guid_pk").replace(/[{}]/g, "");
        DebugLogString("Sending CiiMS Payload script: catched the IM closure event on workstation!");

        var CiiMSDataRaw = getAllDataFromSQLCore(event_uuid);

        var CiiMSDataSplitted = CiiMSDataRaw.split("|");

        var CiiMSPayload = "{\\\"eventDetails\\\":{\\\"uuid\\\":\\\"" + CiiMSDataSplitted[1] + "\\\",\\\"eventFullTimestamp\\\":\\\"" + CiiMSDataSplitted[2] + "\\\",\\\"siteId\\\":\\\"" + CiiMSDataSplitted[3] + "\\\",\\\"zoneId\\\":\\\"" + CiiMSDataSplitted[5] + "\\\",\\\"eventType\\\":\\\"Burglary\\\",\\\"transmitterId\\\":\\\"3856\\\",\\\"responseDispatched\\\":\\\"Yes\\\",\\\"subcontractorDispatched\\\":\\\"" + CiiMSDataSplitted[10] + "\\\",\\\"responseReferenceNumberReceived\\\":\\\"" + CiiMSDataSplitted[11] + "\\\",\\\"responseReferenceNumber\\\":\\\"3856\\\",\\\"acknowledgeDateTime\\\":\\\"" + CiiMSDataSplitted[8] + "\\\",\\\"dispatchDateTime\\\":\\\"" + CiiMSDataSplitted[13] + "\\\",\\\"arrivalDateTime\\\":\\\"" + CiiMSDataSplitted[14] + "\\\",\\\"category\\\":\\\"" + CiiMSDataSplitted[15] + "\\\",\\\"discoveryMethod\\\":\\\"" + CiiMSDataSplitted[16] + "\\\",\\\"reportedBy\\\":\\\"" + CiiMSDataSplitted[17] + "\\\",\\\"rootCause\\\":\\\"" + CiiMSDataSplitted[18] + "\\\",\\\"investigatorRequired\\\":\\\"" + CiiMSDataSplitted[19] + "\\\",\\\"investigator\\\":\\\"" + CiiMSDataSplitted[20] + "\\\",\\\"reactions\\\":\\\"" + CiiMSDataSplitted[21] + "\\\",\\\"response24\\\":\\\"" + CiiMSDataSplitted[22] + "\\\"}}";
        ...
        if (!empty(sending)) {
            ...
            if (json.statusCode != 0) {			// if there are errors, show a popup
                // messageAction("CiiMS notification result of site " + CiiMSDataSplitted[3] + "\\r\\nReceived status code : 200 " "\\r\\nReceived status message: Sucess!" \ "", slave_id);
                var statuscode = "200";
                var statusmessage = "Success!";
                var supportmsg = "Payload sent!";
                messageAction("CiiMS notification result of site : " + CiiMSDataSplitted[3] + "\\r\\nReceived status code : " + statuscode + "\\r\\nReceived status message: " + statusmessage + "\\r\\nReceived support message: " + supportmsg + "", "AXXONDEMO1");
            }
        } else {
            DebugLogString("No Computer!");
            messageAction("CiiMS notification result of site: " + CiiMSDataSplitted[3] + ":\\r\\n Message was not sent!\\r\\n Technical data: variable is \"" + sending + "\"", "AXXONDEMO1");
        }
    }
};

function doSendCiiMSReport(data) {
    var execution_string = "curl " + JSON_header + " " + APIKey + " " + handler + " " + deviceType + " " + endpoint + " --data \"" + data + "\"";
...
function getAllDataFromSQLCore(event_uuid) {
    ...
    var sqlQuery = "SELECT TOP 1 * FROM [dbo].[ciims_messages]  ORDER by sent_timestamp DESC;\""; //WHERE uuid = '" + event_uuid + "'
