...
var endpoint = "https://us-central1-response24-sa-prod.cloudfunctions.net/bidvest/action-tower/v1";
var timeout = 20000;
...
if (Event.SourceType == "MACRO" && Event.Action == "RUN" && Event.SourceId == 1003) {
    ...
    DebugLogString(timestamp + "; R24 update: starting R24 dispatch update for site " + towerId + " by computer " + computer + "...");
    ...
    if (!empty(Event.GetParam("number")) && (!empty(computer)) && (!empty(referenceId))) {
        ...
        var cmd2 = "curl --url \"" + endpoint + "" + Itv_var("T" + Event.GetParam("number") + "_r24_key") + "\"";
        ...
        DoReactStr("OPCIE", opcieId, "FUNC", "func_name<setState>,state<" + json2.data[0].state + ">,message<" + wholeArrayofMessages + ">,key<" + json2.data[0].key + ">");
    }
};
