...
if (Event.SourceType == "MACRO" && Event.Action == "RUN" && Event.SourceId == 1001) {
    ...
    if (!empty(towerId) && (!empty(computer)) && (Itv_var("Failover_flag") == "True")) {
        ...
        if (json.referenceId === undefined || json.referenceId === null) {
            messageAction(executionServer + ": dispatch request for site \"" + towerId + "\" was unsuccessful, retrying...", computer);
        ...
        DebugLogString("R24 dispatch: sending this reaction to OPCIE: DoReactStr(\"OPCIE\",1,\"FUNC\",\"func_name<setState>,state<" + json2.data[0].state + ">,key<" + json.referenceId + ">\");");

        for (i = 0; i < opcieId.length; i++) {
            DoReactStr("OPCIE", opcieId, "FUNC", "func_name<setDispatchTime>,dispatchTime<" + Itv_var("T" + Event.GetParam("number") + "_r24_dispatchTime") + ">,fullTimestamp<" + Itv_var("T" + Event.GetParam("number") + "_r24_dispatchFullTime") + ">");
            DoReactStr("OPCIE", opcieId, "FUNC", "func_name<setState>,state<" + json2.data[0].state + ">,referenceId<" + json.referenceId + ">");
            DebugLogString("R24 dispatch: starting autoUpdate with: " + json2.data[0].state + " and " + json2.referenceId);
            DoReactStr("OPCIE", opcieId, "FUNC", "func_name<autoUpdate>,state<" + json2.data[0].state + ">,key<" + json2.referenceId + ">,computer<" + computer + ">");
        }
