...
if (Event.SourceType == "MACRO" && Event.Action == "RUN" && Event.SourceId == 1002) {
    ...
    if (!empty(Event.GetParam("number")) && (!not_found) && (!empty(referenceId) && Itv_var("Failover_flag") == "True")) {
        ...
        for (i = 0; i < opcieId.length; i++) {
            DoReactStr("OPCIE", opcieId[i], "FUNC", "func_name<clearDispatchTimes>,dispatchTime<>");
            DoReactStr("OPCIE", opcieId[i], "FUNC", "func_name<setState>,state<Moved to history>");	// sending the closed state to HTML page
        }
    }
};
